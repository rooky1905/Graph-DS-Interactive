...
class Graph:
  ...
  def explore(self):
    ...
    while current_room != 'treasure room':
      ...
      for items,weight in node.edges.items():
        key = items[0]
        print("enter {0} for {1}: {2} cost".format(key, items, weight))
...
def build_graph():
  ...
  graph.add_edge(ante_chamber, entrance, 7)
  graph.add_edge(kings_room, entrance, 3)
